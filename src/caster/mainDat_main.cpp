...
void mainDatClass::Sound(){
	mainDatClass* dat = this;
	//sound
	if( dat->waveFlg ){
...
//	Sleep(100);
	...
	memset( &myGameOpt, 0, sizeof(myGameOpt) );
	memset( &enGameOpt, 0, sizeof(enGameOpt) );
	memset( &dataGameOpt, 0, sizeof(dataGameOpt) );
	{
		BYTE defaultDamageLevel = 2;
		myGameOpt.A.damageLevel = defaultDamageLevel;
		myGameOpt.B.damageLevel = defaultDamageLevel;
		enGameOpt.A.damageLevel = defaultDamageLevel;
		enGameOpt.B.damageLevel = defaultDamageLevel;
		dataGameOpt.A.damageLevel = defaultDamageLevel;
		dataGameOpt.B.damageLevel = defaultDamageLevel;
	}
...
//	srand( (unsigned)time( NULL ) );
	...
	case 11:
		if (menuData.mode == 11) {
			//Instant Lobby
			SOCKADDR_IN standbyTemp;
			memset(&standbyTemp, 0, sizeof(standbyTemp));
			
			cout << "debug : display button is CTRL" << endl;
			...
			for(;;){
				if(GetEsc()) return 1;
				if(GetKeyState(VK_CONTROL)<0 && counter > 10){
					counter = 0;
					cout << endl
						 << "<Status>" << endl;
					cout << "Standby[0] is " << inet_ntoa(Standby[0].sin_addr) << endl;
					cout << "Standby[1] is " << inet_ntoa(Standby[1].sin_addr) << endl;
				}
				if(counter < 100) counter++;
				
				if(Standby[0].sin_addr.s_addr && Standby[1].sin_addr.s_addr){
					WaitForSingleObject( hMutex, INFINITE );
					standbyTemp = Standby[0];
					ReleaseMutex( hMutex );
					break;
				}
				Sleep(100);
	...
	case 8 :
		//Receive Test signal
		{
			...
			addrTemp.sin_addr.s_addr = get_address(menuData.targetIP, &addrTemp.sin_port);
...
#ifdef GUI_CONSOLE
				if(N_Gui::g_gui){
					::PostMessage(N_Gui::g_gui->hWnd, WM_COMMAND, N_Gui::ID_NOTICE_CASTER_GOTO_DELAY_INPUT, (LPARAM)(int)(delay * 10));
				}
#endif
				cout.setf(ios::fixed, ios::floatfield);
				cout	<< endl
...
#ifdef GUI_CONSOLE
			if(N_Gui::g_gui){
				::PostMessage(N_Gui::g_gui->hWnd, WM_COMMAND, N_Gui::ID_NOTICE_CASTER_GOTO_DELAY_WAIT, 0);
			}
#endif
			cout	<< endl
				<< "<Delay>" << endl
