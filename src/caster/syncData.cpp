...
int syncDataClass::TestSyncData(DWORD gameTime ){
	...
	if( HereData->HPA != AwayData->HPA ) return 0xF;
	if( HereData->HPB != AwayData->HPB ) return 0xF;
...
int syncDataClass::GetSyncDataHere(DWORD gameTime, DWORD *HPA, DWORD *HPB, DWORD *XA, DWORD *XB) {
	if( gameTime%20) return 3;
	
	syncDataArrayStruct* HereData;
	if( Here.GetSyncDataAddress(gameTime, &HereData ) ) return 2;
	
	if( !( HereData->HPA && HereData->HPB && HereData->XA && HereData->XB ) ) return 1;
