...
unsigned __stdcall N_Caster::boosterThread(void* Address){
	if( !Address ) return 1;
...
//			cout << "debug : booster find th075" << endl;
			...
			N_Booster::boosterDatClass boosterDatA(0xA);
			N_Booster::boosterDatClass boosterDatB(0xB);
...
unsigned __stdcall N_Caster::manageThread(void* Address){
	if( !Address ) return 1;
	mainDatClass* dat = (mainDatClass*)Address;
	#if debug_mode_thread
		WaitForSingleObject( dat->hPrintMutex, INFINITE );
...
unsigned __stdcall N_Caster::th075Thread(void* Address){
	mainDatClass* dat = (mainDatClass*)Address;
	if( !dat ) return 1;
	#if debug_mode_thread
		WaitForSingleObject( dat->hPrintMutex, INFINITE );
...
unsigned __stdcall N_Caster::sendThread(void* Address){
	mainDatClass* dat = (mainDatClass*)Address;
	if( !dat ) return 1;
...
unsigned __stdcall N_Caster::recvThread(void* Address){
	mainDatClass* dat = (mainDatClass*)Address;
	if(!dat) return 1;
...
//										cout << "debug : set syncData Away" << endl;
						...
						case cmd_gameInfo :
							...
							break;
						case res_gameOpt :
							memcpy( &(dat->enGameOpt), recvData, sizeof(dat->enGameOpt) );
							dat->gameOptRecvFlg = 1;
							break;
						case cmd_gameOpt :
							memcpy(data, &dat->myGameOpt, sizeof(dat->myGameOpt));
							dat->SendCmdR( dest_away, res_gameOpt, data, sizeof(dat->myGameOpt) );
							dat->gameOptRecvFlg = 1;
							break;
						case cmd_delay :
...
//														cout << "zSize : " << bufSize << endl;
						...
						case cmd_dataInfo :
							...
							break;
						case cmd_dataOpt :
							memcpy(data, &dat->dataGameOpt, sizeof(dat->dataGameOpt));
							dat->SendCmdR( &addr, res_dataOpt, data, sizeof(dat->dataGameOpt));
							break;
						case cmd_req_palette: {
							int len = 2;
							data[0] = recvData[0];
						...
						//Rootからの通信
						if( *command != res_dataInfo && *command != res_dataOpt && *command != cmd_exit ){
...
//							cout << "debug : cmd_seek_leaf ( Root )" << endl;
						...
						case res_dataInfo :
							...
							break;
						case res_dataOpt :
							memcpy( &(dat->dataGameOpt), recvData, sizeof(dat->dataGameOpt) );
							break;
						case res_join :
...
//							cout << "debug : res_echoes" << endl;
						...
						case cmd_standby :
							if( addr.sin_addr.s_addr == dat->Standby[0].sin_addr.s_addr
								...
								if( dat->Standby[1].sin_addr.s_addr
								    && (!dat->relayControlFlg || dat->relaySpectateMode)){
									WaitForSingleObject( dat->hMutex, INFINITE );
