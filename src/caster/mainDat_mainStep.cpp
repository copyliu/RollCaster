...
class Client{
public:
	int csend(char *hostname,int port,char *message){
						...
						if( ( myInfo.terminalMode == mode_root || myInfo.terminalMode == mode_debug ) && uMsg ){
							...
							*(DWORD*)wParamArray = 0;
							...
							if((this->ran_flag == 1) && (myInfo.terminalMode != mode_debug)){
								...
								char ran_passtmp[256];
								char ran_enIPtmp[256];
								memset( ran_enIPtmp, 0, sizeof( ran_enIPtmp ) );
								memset( ran_passtmp, 0, sizeof( ran_passtmp ) );
								
								clx::md5 md;
								strncpy( ran_enIPtmp, md.encode(ran_enIP).to_string().c_str()      , 255 );
								strncpy( ran_passtmp, md.encode(this->ran_pass).to_string().c_str(), 255 );
								sprintf(buff,
								        "%s %d %d %d %d %s %s %s\n",
								        this->ran_server,
								        wParamArray[0],
...
//							cout << "debug : send syncData" << endl;
					...
					if( gameTime > 50 ){
						int inputCounter;
						for( inputCounter = 2; inputCounter < 30; inputCounter += 2 ){
							if( inputData.GetInputData( sessionNo, gameTime + offset + 2 + inputCounter, myInfo.playerSide, NULL ) ){
								break;
							}
						}
						...
						if( inputCounter < 20 ){
							BYTE preInput;
							inputData.GetInputData( sessionNo, gameTime + offset + inputCounter, myInfo.playerSide, &preInput );
							if( preInput == diInputA ){
								//drop
							}else{
								...
								inputData.GetInputData( sessionNo, gameTime + offset - 2 + inputCounter, myInfo.playerSide, &sendBuf[2] );
								inputData.GetInputData( sessionNo, gameTime + offset + 0 + inputCounter, myInfo.playerSide, &sendBuf[1] );
								inputData.GetInputData( sessionNo, gameTime + offset + 2 + inputCounter, myInfo.playerSide, &sendBuf[0] );
								*(DWORD*)&sendBufBody[2] = gameTime + offset + 2 + inputCounter;
