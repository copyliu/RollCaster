...
BYTE ManipMenuOpt(BYTE phase, charOptStruct* nowOpt, charOptStruct* destOpt){
	
	if( nowOpt->menuDisp    != destOpt->menuDisp ||
	...
	if(nowOpt->menuDisp) {
		if(nowOpt->menuCursor != destOpt->menuCursor) {
			return key_down;
		}
	}
...
BYTE ManipMenuSub( charInfoStruct* now, charInfoStruct* dest, charOptStruct* nowOpt, charOptStruct* destOpt ){
	if(now->phase == 4) { now->placeTime = (now->placeTime % 50); }

	BYTE optInput = ManipMenuOpt(now->phase, nowOpt, destOpt);
	if(optInput != 0){
		return optInput;
	} else if(nowOpt->menuDisp) {
		return 0;
	}
...
//					ReadProcessMemory( hProcess, (void*)(0x12FD14) , &menuAddress , 4 , NULL );
					...
					if(menuAddress){
						...
						if( menuAddress == menuAddressTemp ){
							...
							//Game Option
							ReadMemory((void*)(menuAddress + 0xEA), &myGameOpt.A.menuDisp, 1);
							ReadMemory((void*)(menuAddress + 0xEB), &myGameOpt.B.menuDisp, 1);
							ReadMemory((void*)(menuAddress + 0xEC), &myGameOpt.A.menuCursor, 1);
							ReadMemory((void*)(menuAddress + 0xED), &myGameOpt.B.menuCursor, 1);
							ReadMemory((void*)0x671434, &myGameOpt.A.damageLevel, 1);
							ReadMemory((void*)0x671435, &myGameOpt.B.damageLevel, 1);
							ReadMemory((void*)0x671438, &myGameOpt.A.autoGuard, 1);
							ReadMemory((void*)0x671439, &myGameOpt.B.autoGuard, 1);

							//debug
//							cout << (WORD)myInfo.place << " " << (WORD)myInfo.placeTime << endl;
//							cout << "myGameOpt.A.menuDisp : " << (WORD)myGameOpt.A.menuDisp << endl;
//							cout << "myGameOpt.B.menuDisp : " << (WORD)myGameOpt.B.menuDisp << endl;
//							cout << "myGameOpt.A.menuCursor : " << (WORD)myGameOpt.A.menuCursor << endl;
//							cout << "myGameOpt.B.menuCursor : " << (WORD)myGameOpt.B.menuCursor << endl;
//							cout << "myGameOpt.A.damageLevel : " << (WORD)myGameOpt.A.damageLevel << endl;
//							cout << "myGameOpt.B.damageLevel : " << (WORD)myGameOpt.B.damageLevel << endl;
//							cout << "myGameOpt.A.autoGuard : " << (WORD)myGameOpt.A.autoGuard << endl;
//							cout << "myGameOpt.B.autoGuard : " << (WORD)myGameOpt.B.autoGuard << endl;
...
//					cout << menuFlg << " " << battleFlgA << " " << battleFlgB << endl;
					...
					if(menuFlg == 3 && battleFlgA == 0 && battleFlgB == 0 ){
						if( myInfo.playerSide == 0xA ){
							...
							if( myInfo.terminalMode == mode_root && enCowCaster < 6 && !gameOptRecvFlg && myInfo.A.phase == 0){
								if(InputA == key_P){
									InputA = 0;
								}
							}
							...
							InputB = ManipMenuSub( &myInfo.B, &enInfo.B, &myGameOpt.B, &enGameOpt.B );
						...
						if( myInfo.playerSide == 0xB ){
							...
							InputA = ManipMenuSub( &myInfo.A, &enInfo.A, &myGameOpt.A, &enGameOpt.A );

							InputB = datA.GetInput();

							if( myInfo.terminalMode == mode_root && enCowCaster < 6 && !gameOptRecvFlg && myInfo.B.phase == 0){
								if(InputB == key_P){
									InputB = 0;
								}
							}
...
//						SendCmd( dest_branch, cmd_dataInfo );
						...
						SendCmd( dest_root, cmd_dataOpt);
					}
					roopCounter++;
					if( roopCounter > 6 ) roopCounter = 0;

					if( menuFlg == 3 && !battleFlgA && !battleFlgB ){
						InputA = ManipMenuSub( &myInfo.A, &dataInfo.A, &myGameOpt.A, &dataGameOpt.A );
						InputB = ManipMenuSub( &myInfo.B, &dataInfo.B, &myGameOpt.B, &dataGameOpt.B );
